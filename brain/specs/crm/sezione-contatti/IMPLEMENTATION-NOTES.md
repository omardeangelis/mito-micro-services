...
- **`/simplify` dopo T1.7 (4 revisori: riuso, semplificazione, efficienza, altitudine).** Applicati:
  ...
  - **`lockCustomer` restituisce la riga bloccata (`LockedCustomer`: id e operatore) e `replaceActiveContact` la riceve invece di `customerId`**, senza bloccare di nuovo. Il tipo impone "prima il lock", toglie un `SELECT … FOR UPDATE` e la rilettura dell'operatore nel cron. Scostamento dall'interfaccia `replaceActiveContact({ customerId, values })` di T1.4, che PR3 deve seguire.
