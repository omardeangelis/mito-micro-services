...
> Documento di riferimento ad alto livello. Il dettaglio operativo di ogni fase è in `brain/chore/migration/fase-N-*.md`.
...
5. **Cron**: 4 job (`update`, `alert`, `priority`, `delete` export) come route Next.js chiamate da script Node con JWT firmato con `CRON_SECRET_KEY`. Schedulati da **8 workflow GitHub Actions** in `.github/workflows/` (prod UTC: update 00:00, delete 00:30, priority 01:30, alert 02:00 — tabella completa in `brain/chore/migration/fase-0/t5-ci-workflows.md`).
