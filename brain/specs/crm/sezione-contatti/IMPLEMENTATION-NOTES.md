...
- **Correzioni dopo l'adversarial review ([[specs/crm/sezione-contatti/REPORT]]).**
  ...
  - **F2 — `LockedCustomer` ha un brand privato** (lo produce solo `lockCustomer`) e `replaceActiveContact` richiede `Tx`: fuori da `transaction()` o senza lock non compila. Un test con `@ts-expect-error` lo verifica in `tsc --noEmit`.
