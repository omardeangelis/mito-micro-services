...
- **Motion-craft pass only — the review bar.** When the rubric chartered a pass on animation/motion code, give that verifier the `design-engineer` agent's review bar as its rubric: the project's `design-engineering` review reference when one ships, otherwise the baseline motion standards in `.agents/agents/design-engineer.md` (exact easing/duration values, `transform`/`opacity` only, reduced-motion handled; findings table + Block/Approve).
